      ...
      <div className="grid gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-6 xl:grid-cols-6 ">
        {data.map((item, idx) => (
          <Button
            ...
            variant="default"
            className={`h-28 animate-in slide-in-from-bottom duration-700 w-full hover:bg-primary/20 rounded-xl border border-border bg-background/80 transition-all flex flex-col items-start justify-between p-2 xl:p-4 hover:scale-95 cursor-pointer`}
          >
            {/* Title */}
            ...
            {/* Footer info */}
            <div className="flex w-full justify-between xl:text-xs text-sm text-text font-semibold">
              <div className="flex flex-col xl:flex-row justify-center items-center gap-2">
                {!isEnergyPage && (
                  <Thermometer className="size-4 xl:size-6 text-primary" />
                )}
                <span>{!isEnergyPage && `${item.avgTemp}°C`}</span>
              </div>
